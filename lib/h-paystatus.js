// Чем кончилась оплата. Спрашиваем у SumUp, а не верим браузеру.
//
//   POST /api/pay-status {device, ref}
//        -> {paid:true, ...что теперь есть у человека}
//        -> {paid:false, status}
//
// Начисляем ровно один раз: запись о платеже переводится в PAID,
// и повторный запрос уже ничего не добавляет.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (rec.plan !== DONATION.id) {
      ...
      // Продление складывается: если срок ещё не вышел, считаем от него.
    ...
    // Чек. Человек заплатил настоящие деньги — у него должно остаться
    // письменное подтверждение, а не только экран в приложении.
