...
//
//   hard  — безопасность. Правило одно: инструкция, как навредить себе. Такая
//           фраза из истории вырезается, а сама история всё равно выходит.
//   craft — ремесло и мягкие границы. Машинные обороты, названные чувства,
//           вопрос-допрос, оценка внешности, «всё исчезло навсегда». Даём редактору
//           попытки, а если не вышло, показываем историю с пометкой.
//   soft  — то, что решает человек при вычитке.
//
// Принцип Василия: запрос родителя всегда получает сказку. Про тело, вес, внешность,
// смерть, болезнь и другие сложные вещи мы говорим просто и по-доброму, а не запрещаем.
//
...
// 1. Тело. Дети спрашивают про вес, рост, живот, кожу, и мы отвечаем просто и по-честному.
// Слова «вес», «толстый», «размер» не запрещены. На вычитку человеку идёт только то, что
// превращает тело в цель или оценку: диеты, счёт калорий, «надо похудеть».
const BODY_TARGETS = [
  'диет', 'калори', 'похуде', 'надо худеть', 'нужно худеть', 'сесть на диету', 'считать калории',
  'lose weight', 'go on a diet', 'on a diet', 'count calories', 'calorie'
];
...
const RULES = [
  { id: 'body-targets', level: 'soft', words: BODY_TARGETS,
    why: 'О теле говорим просто и по-доброму. Диеты, счёт калорий и «правильные» цифры как цель не предлагаем.' },
  { id: 'appearance-judged', level: 'craft', words: APPEARANCE_JUDGEMENT,
    why: 'Тело и внешность героя не оцениваются: ни красивое, ни некрасивое. Отличия называются спокойно, как факт.' },
  { id: 'self-harm-method', level: 'hard', words: SELF_HARM_METHOD,
    why: 'Никаких способов самоповреждения, даже в предупреждении. Фраза вырезается.' },
  { id: 'worldview-ranked', level: 'craft', words: WORLDVIEW_RANKED,
    why: 'Ни одно мировоззрение не названо правдой или выдумкой.' },
  { id: 'promises-disappearance', level: 'craft', words: PROMISES_DISAPPEARANCE,
...
export { findWords, SELF_HARM_METHOD };
