...
// Восстановление буквы «ё» только в бесспорных случаях (её→неё, а не «все»→«всё» —
// то, что осталось двузначным, словарь не трогает). Только для голоса: то, что
// видит и читает ребёнок на экране, остаётся с «е», как написал редактор.
// Словарь читается один раз при первом холодном старте функции (~350 мс),
// дальше живёт в памяти тёплого процесса и ничего не стоит.
const eyo = new Eyo();
eyo.dictionary.set(safeDictionary);
function forVoice(text) {
  const s = String(text || '');
  return /[а-яёА-ЯЁ]/.test(s) ? eyo.restore(s) : s;
}
...
export async function generateVoice({ text, voiceId, settings }) {
  // «Ё» вместо «е» там, где это однозначно: слышит только голос, на экране
  // и в тексте сказки остаётся как было. Длина строки не меняется (одна буква
  // на одну), поэтому метки времени ниже (buildSync) считаются по ней же
  // без сдвига.
  ...
  await meterVoice(VOICE_MODEL, spoken.length);
  return {
    audio: `data:audio/mpeg;base64,${j.audio_base64}`,
    sync: buildSync(spoken, j.alignment)
  };
}
