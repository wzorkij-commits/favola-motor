...
// очистка подписи от мусора речи.
//
// Четыре адреса живут внутри api/record.js, чтобы не занимать лишний слот
...
//   POST /api/polish      {sentences, lang}  -> {sentences}   (для Radio: чистый текст подписи)
//
// Принцип: слова рассказчика не переписываются в звуке и в разборе на сцены —
// картинки строятся по временным меткам того, что человек буквально сказал.
// А вот подпись на экране (только текст, звук не трогаем) в Radio чистится
// от «э-э-э», повторов и оговорок отдельным, более деликатным шагом (polish).
...
import { SCENES_SYSTEM, POLISH_SYSTEM, buildPolishPrompt } from './prompts.js';
...
/* ── 4. очистка подписи от мусора речи (только для Radio) ───────────────
   Звук не трогаем — это шаг только для текста, который человек видит на
   экране под своей собственной, настоящей записью. Если модель не сошлась
   по числу предложений или вовсе отказала, отдаём исходный текст без правки:
   сказка должна собираться всегда, а несколько лишних «э-э-э» не беда. */
...
async function polishText(req, res) {
  ...
  const original = sentences.map(s => clip(typeof s === 'string' ? s : (s && s.text) || '', 600));
  try {
    const raw = await generateText({
      system: POLISH_SYSTEM, prompt: buildPolishPrompt(original, lang), maxTokens: 4000, temperature: 0.2
    });
    const j = jsonFrom(raw);
    const clean = Array.isArray(j.sentences) ? j.sentences.map(s => clip(s, 600)) : null;
    if (clean && clean.length === original.length) {
      return res.status(200).json({ sentences: clean, source: 'llm' });
    }
    return res.status(200).json({ sentences: original, source: 'fallback', why: 'число предложений не сошлось' });
  } catch (e) {
    return res.status(200).json({ sentences: original, source: 'fallback', why: String(e.message || e) });
  }
}

/* ── вход ─────────────────────────────────────────────────────────────── */

const ROUTES = { clean: cleanAudio, transcribe, scenes, polish: polishText };
