// Списание одной сказки — общий счётчик с Favola (одна подписка на оба
// приложения). Приложение зовёт это до того, как начнёт собирать.
//
//   POST /api/spend {device}               ->  {ok:true, ...}      можно собирать, списали
//   POST /api/spend {device, kind:'record'} ->  то же для «Записать сказку»
...
import { meterStory } from './ledger.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await saveUser(u);
    await meterStory('radio-' + (kind || 'story'));
